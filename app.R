...
# Version : 4.0 — Sprints 1+2+3+4+5 (Plantations)
...
# ══════════════════════════════════════════════════════════════
# DONNÉES STATIQUES PLANTATIONS (FAO/ANRPC 2024)
# ══════════════════════════════════════════════════════════════
  ...
  dashboardSidebar(
    ...
    tags$div(
      ...
      sliderInput("decalage","Decalage (semaines)",
                  min=0, max=8, value=0, step=1, post=" sem.", width="100%"),
      tags$hr(style="border-color:#444;"),
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName="dashboard",
        ...
        fluidRow(
          box(title="Prix TSR20 — Historique",status="warning",width=12,height=420,
      ...
      tabItem(tabName="correlations",
        ...
        fluidRow(
          box(title=uiOutput("titre_temporel"),status="warning",width=12,height=450,
        ...
        fluidRow(
          box(title="Interpretation — Analyse de correlation",status="success",width=12,
      ...
      tabItem(tabName="simulateur",
        ...
        fluidRow(
          box(title="Interpretation — RubberSignal Analysis",status="warning",width=12,
              uiOutput("interp_sim"))
        )
      ),
          ...
          box(title="Configurateur de scenarios meteo",status="warning",width=5,height=540,
              ...
              checkboxGroupInput("sc_zones",NULL,
                choices=c("Cote d'Ivoire (15%)"="ci","Thailande (35%)"="th",
                          "Malaisie (25%)"="my","Indonesie (25%)"="id"),
                selected="ci"),
      ...
      tabItem(tabName="plantations",
        fluidRow(
          valueBoxOutput("pl_surface",width=3),
          valueBoxOutput("pl_prod",width=3),
          valueBoxOutput("pl_top",width=3),
          valueBoxOutput("pl_ci",width=3)
        ),
        fluidRow(
          box(title="Carte mondiale des plantations d'hevea",
              status="success",width=8,height=520,
          ...
          box(title="Filtres",status="warning",width=4,height=520,
              ...
              checkboxGroupInput("pl_cont",NULL,
                choices=c("Asie"="Asie","Afrique"="Afrique"),
              ...
              radioButtons("pl_metric",NULL,
                choices=c("Superficie (kha)"="superficie_kha",
                          "Production (kt)"="production_kt",
                          "Part mondiale (%)"="part_mondiale_pct",
                          "Rendement (kg/ha)"="rendement_kg_ha"),
                selected="superficie_kha"),
              tags$hr(style="border-color:#334;"),
              tags$p("Cliquez sur un pays sur la carte pour ses details.",
                     style="color:#888;font-size:12px;"),
              tags$hr(style="border-color:#334;"),
              tags$p("Sources : FAO FAOSTAT 2024, ANRPC Statistical Bulletin 2024",
                     style="color:#666;font-size:11px;"))
        ),
        fluidRow(
          box(title="Superficies par pays (milliers d'hectares)",
              status="primary",width=6,height=400,
              plotlyOutput("pl_g_surf",height="340px")),
          box(title="Production vs Rendement",
              status="info",width=6,height=400,
              plotlyOutput("pl_g_rend",height="340px"))
        ),
        fluidRow(
          box(title="Donnees completes — Plantations mondiales",
...
server <- function(input, output, session) {
  ...
  output$corr_r <- renderValueBox({
    ...
    r <- cs$r
    valueBox(as.character(r),paste0("Pearson r | decalage ",input$decalage," sem."),icon("calculator"),
  ...
  output$g_scatter <- renderPlotly({
    ...
    if(is.null(d)||nrow(d)<3) {
      return(plot_ly() %>% theme_rs() %>% layout(
        annotations=list(list(text="Donnees insuffisantes",x=0.5,y=0.5,
  ...
  output$g_heatmap <- renderPlotly({
    ...
    if(is.null(h)||nrow(h)<3) {
      return(plot_ly() %>% theme_rs() %>% layout(
        annotations=list(list(text="Donnees insuffisantes pour la heatmap",x=0.5,y=0.5,
                              xref="paper",yref="paper",showarrow=FALSE,font=list(color="#aaa",size=14)))))
    }
    ...
    if(nrow(dm)<3) {
      return(plot_ly() %>% theme_rs() %>% layout(
        annotations=list(list(text="Pas assez de donnees completes",x=0.5,y=0.5,
                              xref="paper",yref="paper",showarrow=FALSE,font=list(color="#aaa",size=14)))))
    }
  ...
  output$interp_corr <- renderUI({
    ...
    tags$div(style="padding:15px;",
      ...
      tags$hr(),
      tags$p(paste0("La correlation entre ",nom_signal()," et le TSR20 est ",force," et ",direction,
                    " (r=",r,", n=",n," sem.). ",
  ...
  output$t_mc <- renderTable({
    ...
    r$stats %>% filter(semaine %in% c(4,8,12,26,r$hr)) %>%
      ...
      mutate(Horizon=paste(semaine,"sem."),Bear=round(p10,3),Base=round(moyenne,3),
             Bull=round(p90,3),`P(up)%`=round(prob_hausse,1)) %>%
  ...
  output$interp_sim <- renderUI({
    ...
    tags$div(style="padding:15px;",
      ...
      tags$hr(),
      tags$p(paste0("Monte Carlo (",format(r$ns,big.mark=" ")," sim., GBM Thompson 2000) : biais ",biais,
                    " a 4 semaines — P(hausse) ",round(s4$prob_hausse,1),"%. Scenario central a ",r$hr,
                    " semaines : ",round(r$stats$moyenne[r$hr],3)," USD/kg. IC80 a 12 sem. : [",
                    round(s12$p10,3)," - ",round(s12$p90,3),"] USD/kg."),
             style="color:#aaa;font-size:13px;line-height:1.6;"),
  ...
  output$sc_desc <- renderUI({
    ...
    tags$p(paste0("Choc offre mondiale : ",choc,"% | Zones : ",paste(toupper(zones),collapse="+"),
  ...
  output$sc_base <- renderValueBox({
    r <- sc_res()
    valueBox(paste(round(r$px,4),"USD/kg"),"Prix TSR20 actuel (base)",icon("tag"),color="yellow")
  })
  output$sc_choc <- renderValueBox({
    r <- sc_res(); sc <- r$sc %>% filter(semaine==12)
    valueBox(paste(round(sc$moyenne,3),"USD/kg"),"Scenario central 12 sem.",icon("chart-line"),
             color=if(r$delta12>0)"red" else "green")
  })
  output$sc_delta <- renderValueBox({
    r <- sc_res(); d <- r$delta12
    valueBox(paste0(if(d>0)"+" else "",d," USD/kg"),"Impact vs scenario base",
             icon(if(d>0)"arrow-up" else "arrow-down"),
  ...
  output$sc_g_mc <- renderPlotly({
    ...
    plot_ly() %>%
      ...
      add_lines(x=c(0,12),y=c(r$px,r$px),line=list(color="#fff",dash="dot",width=1),
                name="Prix actuel",hoverinfo="skip") %>%
      theme_rs() %>%
      layout(xaxis=list(title="Semaines",gridcolor="#334",zeroline=FALSE),
  ...
  output$sc_g_comp <- renderPlotly({
    ...
    for (i in seq_along(sc_list)) {
      ...
      fig <- fig %>% add_lines(x=~st$semaine,y=~st$moyenne,
    ...
    fig %>% theme_rs() %>%
      layout(xaxis=list(title="Semaines",gridcolor="#334"),
             yaxis=list(title="Prix TSR20 (USD/kg)",gridcolor="#334",tickformat=".4f",range=list(ymin2,ymax2)),
             annotations=list(list(text=paste0("Delta max : ",round(r$delta12,4)," USD/kg"),
                                   x=0.98,y=0.05,xref="paper",yref="paper",
  ...
  output$sc_interp <- renderUI({
    ...
    tags$div(style="padding:10px;",
      ...
      tags$p(paste0("Scenario : ",tn[r$type]," affectant ",paste(zn[r$zones],collapse=" + "),
                    " (",round(r$pt*100),"% prod. mondiale). Choc offre : ",round(r$choc_offre*100,1),"%. ",
                    "Impact drift : ",round(r$ajust*100,2),"%/an. Resultat : ",impact," de ",abs(d)," USD/kg."),
  ...
  pl_data <- reactive({
    donnees_plantations %>% filter(continent %in% input$pl_cont)
  })

  output$pl_surface <- renderValueBox({
    valueBox(paste(sum(donnees_plantations$superficie_kha),"kha"),
             "Superficie totale mondiale",icon("globe"),color="green")
  })

  output$pl_prod <- renderValueBox({
    valueBox(paste(sum(donnees_plantations$production_kt),"kt"),
             "Production totale mondiale",icon("industry"),color="blue")
  })

  output$pl_top <- renderValueBox({
    d <- donnees_plantations %>% arrange(desc(production_kt))
    valueBox(d$pays[1],paste0("1er producteur — ",d$production_kt[1]," kt"),
  ...
  output$pl_ci <- renderValueBox({
    ...
    valueBox(paste0("#",rk),
             paste0("CI — ",ci$production_kt," kt — ",ci$part_mondiale_pct,"% mondial"),
             icon("flag"),color="orange")
  })

  output$carte_pl <- renderLeaflet({
    d <- pl_data(); if(nrow(d)==0) return(leaflet() %>% addTiles())
    metric  <- input$pl_metric
    valeurs <- d[[metric]]
    rayons  <- sqrt(valeurs/max(valeurs,na.rm=TRUE))*60
    couleurs <- colorFactor(palette=c("#e67e22","#3498db"),domain=c("Asie","Afrique"))
    labels_unite <- switch(metric,
      superficie_kha="kha",production_kt="kt",
      part_mondiale_pct="%",rendement_kg_ha="kg/ha")
    leaflet(d) %>%
      addProviderTiles(providers$CartoDB.DarkMatter) %>%
      addCircleMarkers(
        lng=~lon, lat=~lat,
        radius=rayons,
        color=~couleurs(continent),
        fillColor=~couleurs(continent),
        fillOpacity=0.7,
        stroke=TRUE, weight=2,
        popup=~paste0(
          "<b>",pays,"</b><br>",
          "Superficie : ",superficie_kha," kha<br>",
          "Production : ",production_kt," kt<br>",
          "Rendement : ",rendement_kg_ha," kg/ha<br>",
          "Age moyen : ",age_moyen_ans," ans<br>",
          "Grade : ",qualite_grade,"<br>",
          "Part mondiale : ",part_mondiale_pct,"%"),
        label=~paste0(pays," — ",valeurs," ",labels_unite)
      ) %>%
  ...
  output$pl_g_surf <- renderPlotly({
    ...
    couleurs_cont <- ifelse(d$continent=="Afrique","#e67e22","#3498db")
    plot_ly(d,x=~reorder(pays,superficie_kha),y=~superficie_kha,type="bar",
            marker=list(color=couleurs_cont,line=list(color="#fff",width=0.5)),
            hovertemplate="%{x}<br>Superficie : %{y} kha<extra></extra>") %>%
      theme_rs() %>%
      layout(xaxis=list(title="",tickangle=-45,gridcolor="#334"),
             ...
             annotations=list(list(text="Orange = Afrique | Bleu = Asie",
  ...
  output$pl_g_rend <- renderPlotly({
    ...
    plot_ly() %>%
      add_bars(x=~reorder(d$pays,d$rendement_kg_ha),y=~d$rendement_kg_ha,
               name="Rendement (kg/ha)",
               marker=list(color="#2ecc71",line=list(color="#fff",width=0.5)),
               hovertemplate="%{x}<br>Rendement : %{y} kg/ha<extra></extra>") %>%
      add_lines(x=~reorder(d$pays,d$rendement_kg_ha),
                ...
                hovertemplate="%{x}<br>Prod. normalisee : %{y:.0f}<extra></extra>") %>%
      theme_rs() %>%
      layout(xaxis=list(title="",tickangle=-45,gridcolor="#334"),
             yaxis=list(title="Rendement (kg/ha)",gridcolor="#334",side="left"),
             yaxis2=list(title="Production (normalisee)",overlaying="y",side="right",gridcolor="#334"),
  ...
  output$pl_table <- renderTable({
    donnees_plantations %>%
      ...
      mutate(Rang=row_number(),Pays=pays,Continent=continent,
             ...
             `Rendement kg/ha`=rendement_kg_ha,`Age moyen (ans)`=age_moyen_ans,
             `Part mondiale %`=part_mondiale_pct,Grade=qualite_grade) %>%
      select(Rang,Pays,Continent,`Superficie kha`,`Production kt`,
             `Rendement kg/ha`,`Age moyen (ans)`,`Part mondiale %`,Grade)
  },striped=TRUE,hover=TRUE,bordered=FALSE,style="color:#eee;background:#16213e;")
